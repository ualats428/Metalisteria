...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $calle = $_POST['calle']; // En BD es 'direccion'
    $numero = $_POST['numero'];
    $piso = $_POST['piso'];
    $cp = $_POST['cp'];       // En BD es 'codigo_postal'
    $localidad = $_POST['localidad']; // En BD es 'ciudad'
    ...
    if ($email !== $email_confirm) {
        ...
        try {
            // Preparamos la consulta SQL coincidiendo con tu tabla 'clientes'
            ...
            $stmt->execute([
                ...
                ':apellidos' => $apellidos,
                ':email' => $email,
                ':password' => $password, // Guardamos en texto plano para ser compatible con tu login actual
                ':dni' => $dni,
                ':telefono' => $telefono,
                ':direccion' => $calle,
                ':numero' => $numero,
            ...
            // --- 4. REGISTRO EXITOSO: INICIAMOS SESIÓN AUTOMÁTICAMENTE ---
            
            // Recuperamos el ID del usuario recién creado
            $nuevo_id = $conn->lastInsertId();

            // Guardamos sesión en servidor
            $_SESSION['usuario_id'] = $nuevo_id;
            $_SESSION['usuario_nombre'] = $nombre;
            $_SESSION['usuario_rol'] = 'cliente';

            // --- 5. PUENTE HÍBRIDO (Igual que en Login) ---
            // Esto actualiza el auth.js y redirige
            echo "<script>
            ...
            // Error común: El email o DNI ya existen (por el UNIQUE en la BD)
            if ($e->getCode() == 23000) {
                $error = "El correo electrónico o el DNI ya están registrados.";
            } else {
                $error = "Hubo un error en el registro: " . $e->getMessage();
            }
        }
    }
}
